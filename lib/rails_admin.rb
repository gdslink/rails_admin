...
module RailsAdmin
  ...
  def self.scope_with(&block)
    ActiveSupport::Deprecation.warn("'#{self.name}.scope_with { }' is deprecated, use 'RailsAdmin.config{|c| c.scope_with }' instead", caller)
    self.config {|c| c.scope_with(&block) }
  end  
